#line 1 "..\\code\\uCGUI\\Widget\\FRAMEWIN.c"
...
#line 21 "..\\code\\uCGUI\\Widget\\FRAMEWIN.c"
...
#line 22 "..\\code\\uCGUI\\Widget\\FRAMEWIN.c"
...
#line 23 "..\\code\\uCGUI\\Widget\\FRAMEWIN.c"
#line 1 "..\\code\\GUI\\GUI_inc\\GUI_Protected.h"
...
#line 24 "..\\code\\uCGUI\\Widget\\FRAMEWIN.c"
#line 25 "..\\code\\uCGUI\\Widget\\FRAMEWIN.c"
...
static signed short FRAMEWIN__MinVisibility = 5;
...
static void _SetActive(FRAMEWIN_Handle hObj, int State) {
  ...
  if        (State && !(pObj->Flags & (1<<3))) {
    pObj->Flags |= (1<<3);
    WM_InvalidateWindow(hObj);
  } else if (!State && (pObj->Flags & (1<<3))) {
    pObj->Flags &= ~(1<<3);
    WM_InvalidateWindow(hObj);
  }
}
...
static void _Paint(FRAMEWIN_Obj* pObj) {
  GUI_HWIN hWin = WM_GetActiveWindow();
  const char* pText = 0;
...
static void _FRAMEWIN_Callback (WM_MESSAGE *pMsg) {
  ...
  FRAMEWIN_Obj* pObj = (FRAMEWIN_Obj*) GUI_ALLOC_h2p(hWin);
  ...
  case 13:
    ...
    if (!(pObj->Flags & (1<<3))) {
      ...
      const GUI_PID_STATE * pState;
      pMsgOrg = (const WM_MESSAGE*)pMsg->Data.p;       
...
static void FRAMEWIN__cbClient(WM_MESSAGE* pMsg) {
  ...
  GUI_HWIN hParent = WM_GetParent(pMsg->hWin);
  FRAMEWIN_Obj* pObj = (FRAMEWIN_Obj*) GUI_ALLOC_h2p(hParent);
  WM_CALLBACK* cb = pObj->cb;
  switch (pMsg->MsgId) {
  case 0x000F:
    if (pObj->Props.ClientColor != 0xFFFFFFF) {
                                  ...
                                  int WinFlags, int ExFlags, int Id, const char* pTitle, WM_CALLBACK* cb)
{
  ...
  if (hObj) {
    ...
    WIDGET__Init(&pObj->Widget, Id, (1<<4) | (1<<5));
    ...
    pObj->hClient = WM_CreateWindowAsChild(Pos.rClient.x0,Pos.rClient.y0,
                                           ...
                                           Pos.rClient.y1 - Pos.rClient.y0 +1,
                                           hObj, 
                                           (1<<7) | (1<<9) | (1<<10) | (1<<8) | (1<<1) | (1<<12), 
                                           FRAMEWIN__cbClient, 0);
    


 
    if ((WinFlags & ((1<<2) | ((1<<13)))) == 0) {
