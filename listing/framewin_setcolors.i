#line 1 "..\\code\\uCGUI\\Widget\\FRAMEWIN_SetColors.c"
...
#line 21 "..\\code\\uCGUI\\Widget\\FRAMEWIN_SetColors.c"
...
void FRAMEWIN_SetBarColor(FRAMEWIN_Handle hObj, unsigned Index, GUI_COLOR Color) {
  if (hObj) {
    ...
    if (Index < (sizeof(pObj->Props . aBarColor) / sizeof(pObj->Props . aBarColor[0]))) {
...
void FRAMEWIN_SetClientColor(FRAMEWIN_Handle hObj, GUI_COLOR Color) {
  if (hObj) {
    ...
    if (pObj->Props.ClientColor != Color) {
      ...
      WM_InvalidateWindow(pObj->hClient);
    }
    ;
  }
}
