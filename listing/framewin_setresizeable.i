#line 1 "..\\code\\uCGUI\\Widget\\FRAMEWIN_SetResizeable.c"
...
#line 21 "..\\code\\uCGUI\\Widget\\FRAMEWIN_SetResizeable.c"
#line 1 "..\\code\\uCGUI\\Widget\\FRAMEWIN_Private.h"
...
#line 24 "..\\code\\uCGUI\\Widget\\FRAMEWIN_Private.h"
...
#line 25 "..\\code\\uCGUI\\Widget\\FRAMEWIN_Private.h"
...
#line 26 "..\\code\\uCGUI\\Widget\\FRAMEWIN_Private.h"
#line 1 "..\\code\\uCGUI\\Widget\\GUI_HOOK.h"
...
#line 24 "..\\code\\uCGUI\\Widget\\GUI_HOOK.h"
...
#line 27 "..\\code\\uCGUI\\Widget\\FRAMEWIN_Private.h"
...
typedef struct {
  ...
  signed short                          TitleHeight;
  signed short                          BorderSize;
  signed short                          IBorderSize;
} FRAMEWIN_PROPS;
...
typedef struct {
  ...
  GUI_HWIN hText;
  GUI_RECT rRestore;
  signed short TextAlign;
  unsigned short Flags;
  GUI_HWIN hFocussedChild;           
...
int             FRAMEWIN_GetTitleHeight   (FRAMEWIN_Handle hObj);
void            FRAMEWIN_MinButtonSetState(GUI_HWIN hButton, int State);
void            FRAMEWIN_MaxButtonSetState(GUI_HWIN hButton, int State);

#line 22 "..\\code\\uCGUI\\Widget\\FRAMEWIN_SetResizeable.c"
...
#line 41 "..\\code\\uCGUI\\Widget\\FRAMEWIN_SetResizeable.c"

#line 48 "..\\code\\uCGUI\\Widget\\FRAMEWIN_SetResizeable.c"
...
static const GUI_COLOR _ColorsCursor[] = {
...
static void _SetResizeCursor(int Mode) {
  const GUI_CURSOR  * pNewCursor = 0;
  if (Mode) {
    int Direction;
    Direction = Mode & ((1<<0) | (1<<1));
    if (Direction == (1<<0)) {
      pNewCursor = &_ResizeCursorH;
    } else if (Direction == (1<<1)) {
      pNewCursor = &_ResizeCursorV;
    } else {
      Direction = Mode & ((1<<2) | (1<<3));
      if ((Direction == ((1<<2) | (1<<3))) || !Direction) {
...
static void _SetCapture(FRAMEWIN_Handle hWin, int x, int y, int Mode) {
  if ((_CaptureFlags & (1<<2)) == 0) {
    _CaptureX = x;
  }
  if ((_CaptureFlags & (1<<3)) == 0) {
    _CaptureY = y;
  }
  if (Mode) {
...
static void _ChangeWindowPosSize(FRAMEWIN_Handle hWin, int* px, int* py) {
  ...
  if (_CaptureFlags & (1<<0)) {
    dx = (_CaptureFlags & (1<<2)) ? (_CaptureX - *px) : (*px - _CaptureX);
  }
  if (_CaptureFlags & (1<<1)) {
    dy = (_CaptureFlags & (1<<3)) ? (_CaptureY - *py) : (*py - _CaptureY);
  }
  ...
  if (_CaptureFlags & (1<<2)) {
    WM_MoveWindow(hWin, -dx, 0);
  }
  if (_CaptureFlags & (1<<3)) {
...
static int _CheckReactBorder(FRAMEWIN_Handle hWin, int x, int y) {
  ...
  if ((x >= 0) && (y >= 0) && (x <= r.x1) && (y <= r.y1)) {
    Mode |= _CheckBorderX(x, r.x1, 3);
    if (Mode) {
      Mode |= _CheckBorderY(y, r.y1, 4 * 3);
    } else {
      Mode |= _CheckBorderY(y, r.y1, 3);
      if (Mode) {
        Mode |= _CheckBorderX(x, r.x1, 4 * 3);
      }
    }
  }
  return Mode;
...
static int _OnTouch(FRAMEWIN_Handle hWin, WM_MESSAGE* pMsg) {
  ...
  if (pState) {   
    ...
    if (pState->Pressed == 1) {
      if (_CaptureFlags & ((1<<0) | (1<<1) | (1<<2) | (1<<3))) {
...
#line 417 "..\\code\\uCGUI\\Widget\\FRAMEWIN_SetResizeable.c"
    } else if (WM_HasCaptured(hWin)) {
      _CaptureFlags &= ~(((1<<0) | (1<<1) | (1<<2) | (1<<3)));
...
#line 451 "..\\code\\uCGUI\\Widget\\FRAMEWIN_SetResizeable.c"




 
#line 480 "..\\code\\uCGUI\\Widget\\FRAMEWIN_SetResizeable.c"
...
void FRAMEWIN_SetResizeable(FRAMEWIN_Handle hObj, int State) {
  if (hObj) {
    FRAMEWIN_Obj* pObj;
    ;
    pObj = (FRAMEWIN_Obj*) GUI_ALLOC_h2p(hObj);
    if (pObj) {
      if (State) {
