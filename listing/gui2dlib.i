#line 1 "..\\code\\uCGUI\\Core\\GUI2DLib.c"
...
#line 21 "..\\code\\uCGUI\\Core\\GUI2DLib.c"
...
#line 22 "..\\code\\uCGUI\\Core\\GUI2DLib.c"
#line 23 "..\\code\\uCGUI\\Core\\GUI2DLib.c"
...
#line 130 "..\\code\\uCGUI\\Core\\GUI2DLib.c"
  i = angle>>6;
  {
    Faktor = (unsigned short)((1<<6)-(angle&((1<<6)-1)));
    t = aSin[i]*(unsigned long)Faktor;
    if (Faktor !=(1<<6)) {
...
static int _atan2(signed long x, signed long y) {
  unsigned char q =0;
  int angle;
...
#line 279 "..\\code\\uCGUI\\Core\\GUI2DLib.c"




 
static void _CalcOrto(int xDiff, int yDiff, signed long r, int*px, int*py) {
  signed long x,y;
  ...
  Angle+= (2*512);
  x = ( r*(signed long)GUI_cos(Angle));
  y = ( r*(signed long)GUI_sin(Angle));
...
void GL_DrawLine(int x0, int y0, int x1, int y1) {
  if (GUI_Context.PenSize ==1) {
    ...
    if (xdiff | ydiff) {    
      ...
      _CalcOrto(x0-x1,y0-y1, (signed long)(GUI_Context.PenSize-1), &xOff, &yOff);
...
void GUI_DrawLineTo(int x, int y) {
  ;
    ...
    x += GUI_Context . xOff; y += GUI_Context . yOff;
    { if (WM__InitIVRSearch(0)) do {; {
...
#line 612 "..\\code\\uCGUI\\Core\\GUI2DLib.c"
  ...
  if (GUI_Context.LineStyle == (0)) {
    for (i=0; i<=xdiff; i++) {
      signed long l = ((signed long)ydiff)*i+xdiffby2;
      int y = (((l>0) ? l : -l)<32767) ? (y0 + ((int)l)/xdiff) : (y0 + l/xdiff);
      if (!Swapped)
        GUI_Context . pLCD_HL->pfDrawPixel(x0+i, y);
      else
        GUI_Context . pLCD_HL->pfDrawPixel(y, x0+i);
    }
...
void GL_DrawPoint(int x, int y) {
  if (GUI_Context.PenSize == 1) {
    GUI_Context . pLCD_HL->pfDrawPixel(x, y);
  } else {
