#line 1 "..\\code\\uCGUI\\Core\\GUI_AddDecShift.c"
...
#line 21 "..\\code\\uCGUI\\Core\\GUI_AddDecShift.c"
#line 22 "..\\code\\uCGUI\\Core\\GUI_AddDecShift.c"
...
#line 23 "..\\code\\uCGUI\\Core\\GUI_AddDecShift.c"
...
const unsigned long GUI_Pow10[10] = {
  1 , 10, 100, 1000, 10000,
...
static int _Check_NegLong(signed long *pv, char**ps) {
  if (*pv < 0) {
    *(*ps)++ = '-';
    *pv = -*pv;
    return 1;
...
int GUI_Long2Len(signed long vSign) {
  int Len = 1;
  signed long v = (vSign > 0) ? vSign : -vSign;
  while (( ((unsigned long)v) >= GUI_Pow10[Len]) && (Len < 9)) {
    Len++;
  }
  if (vSign < 0) {
		Len++;
...
#line 115 "..\\code\\uCGUI\\Core\\GUI_AddDecShift.c"
  if (Shift) {
    Len--;
  }
#line 124 "..\\code\\uCGUI\\Core\\GUI_AddDecShift.c"
  if ((unsigned long)v >= GUI_Pow10[Len])
	v = GUI_Pow10[Len] - 1;
  while (Len) {
    if (Len-- == Shift)
      *(*ps)++ = GUI_DecChar;
