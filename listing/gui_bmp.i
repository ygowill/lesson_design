#line 1 "..\\code\\uCGUI\\Core\\GUI_BMP.c"
...
#line 21 "..\\code\\uCGUI\\Core\\GUI_BMP.c"
...
#line 23 "..\\code\\uCGUI\\Core\\GUI_BMP.c"
...
static int _DrawBitmap_Pal(const unsigned char * pData, int x0, int y0, int XSize, int YSize, int Bpp, int NumColors) {
  ...
  for (i = 0; i < NumColors; i++) {
    ...
    r = *(pData + 2);
    pData += 4;
    LCD__aConvTable[i] = LCD_Color2Index(((unsigned long)b << 16) | (g << 8) | r);
  }
  switch (Bpp) {
    case 1:
...
static int _DrawBitmap_24bpp(const unsigned char * pData, int x0, int y0, int XSize, int YSize) {
  ...
  for (; (y < YSize) && (y >= 0); y += Step) {
    for (x = 0; x < XSize; x++) {
      ...
      g = *(pColor + 1);
      r = *(pColor + 2);
      LCD_SetPixelIndex(x0 + x, y0 + y, LCD_Color2Index(((unsigned long)b << 16) | (g << 8) | r));
    }
    pData += BytesPerLine;
  }
  return 0;
...
int GUI_BMP_Draw(const void * pBMP, int x0, int y0) {
  ...
  unsigned long ClrUsed, Compression;
  int NumColors;
  const unsigned char * pSrc = (const unsigned char *)pBMP;
  Type        = GUI__Read16(&pSrc);  
  pSrc += 12;                    
