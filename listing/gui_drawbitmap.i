#line 1 "..\\code\\uCGUI\\Core\\GUI_DrawBitmap.c"
...
#line 21 "..\\code\\uCGUI\\Core\\GUI_DrawBitmap.c"
...
#line 22 "..\\code\\uCGUI\\Core\\GUI_DrawBitmap.c"
...
void GL_DrawBitmap(const GUI_BITMAP  * pBitmap, int x0, int y0) {
  ...
  const GUI_LOGPALETTE  * pPal;
  pPal = pBitmap->pPal;
  PrevDraw = GUI_SetDrawMode(0);   
  GUI_SetDrawMode((pPal && pPal->HasTrans) ? (PrevDraw|(1<<1)) : PrevDraw &(~(1<<1)));
  if (pBitmap->pMethods) {

      pBitmap->pMethods->pfDraw(x0, y0, pBitmap->XSize ,pBitmap->YSize, (unsigned char const *)pBitmap->pData, pBitmap->pPal, 1, 1);

  } else {
    const unsigned short* pTrans;
    pTrans = LCD_GetpPalConvTable(pBitmap->pPal);
    if (!pTrans) {
      pTrans = (pBitmap->BitsPerPixel != 1) ? 0 : &GUI_Context . LCD . aColorIndex16[0];
    }
