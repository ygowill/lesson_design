#line 1 "..\\code\\uCGUI\\Core\\GUI_FillPolygon.c"
...
#line 21 "..\\code\\uCGUI\\Core\\GUI_FillPolygon.c"
...
#line 22 "..\\code\\uCGUI\\Core\\GUI_FillPolygon.c"
#line 23 "..\\code\\uCGUI\\Core\\GUI_FillPolygon.c"
...
static void _DrawHLine(int x0, int y, int x1) {
  if (x0 < x1) {
    GUI_Context . pLCD_HL->pfDrawHLine(x0, y, x1);
  } else {
    GUI_Context . pLCD_HL->pfDrawHLine(x1, y, x0);
  }
}
...
static int _CheckYInterSect(int y, int* px, const GUI_POINT*paPoint0, const GUI_POINT*paPoint1) {
  ...
  {
    signed long Mul = (signed long)(x1 - x0) * (signed long)(y - y0);
    if (Mul > 0) {
      Mul += (y1 - y0) >> 1;	           
    } else {
      Mul -= ((y1 - y0) >> 1) - 1;	     
...
static void _Flush(int x0, int y) {
  ...
  for (i=0; i<GL_FP_Cnt; i++) {
    int xNew = _aX[i];
    if (On) {
      GUI_Context . pLCD_HL->pfDrawHLine(x0 + x, y, x0 + xNew);
    }
    On ^= 1;
    x = xNew;
  }
...
static void _AddPoint(int x, int y, int y0, int y1, int xOff, int yOff) {
  if ((y0 ^ y1) >= 0) {
    x += xOff;
    GUI_Context . pLCD_HL->pfDrawHLine(x, y + yOff, x);     
  } else {
    _Add(x);
  }
}
