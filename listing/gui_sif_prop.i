#line 1 "..\\code\\uCGUI\\Core\\GUI_SIF_Prop.c"
...
#line 21 "..\\code\\uCGUI\\Core\\GUI_SIF_Prop.c"
...
static int _GetNumCharAreas(const GUI_FONT  * pFont) {
  unsigned short NumCharAreas;
  const unsigned char * pData;
  pData = (const unsigned char *)pFont->p.pFontData + 14  ;
...
static const unsigned char * _GetpCharInfo(const GUI_FONT  * pFont, unsigned short c) {
  const unsigned char * pCharArea, * pCharInfo;
  int NumCharAreas;
  NumCharAreas = _GetNumCharAreas(pFont);                  
  pCharArea = (const unsigned char *)pFont->p.pFontData 
              + sizeof(GUI_SI_FONT);                       
  pCharInfo = pCharArea
              + sizeof(GUI_SIF_CHAR_AREA) * NumCharAreas;  
  do {
...
static void _GUI_SIF_DispChar(unsigned short c) {
  const unsigned char * pCharInfo, * pData;
  ...
  if (pCharInfo) {
    ...
    pData = (const unsigned char *)GUI_Context.pAFont->p.pFontData + CharInfo.OffData;
    ...
    LCD_DrawBitmap(GUI_Context.DispPosX, GUI_Context.DispPosY,
                   ...
                   pData,
                   &GUI_Context . LCD . aColorIndex16[0]);
...
static int _GUI_SIF_GetCharDistX(unsigned short c) {
  const unsigned char * pCharInfo;
  unsigned short DistX = 0;
  pCharInfo = _GetpCharInfo(GUI_Context.pAFont, c);  
  if (pCharInfo) {
    pCharInfo += 2  ;
...
static void _GUI_SIF_GetFontInfo(const GUI_FONT  * pFont, GUI_FONTINFO * pfi) {
  const unsigned char * pData;
  pData = (const unsigned char *)pFont->p.pFontData + 4  ;
...
static char _GUI_SIF_IsInFont(const GUI_FONT  * pFont, unsigned short c) {
  const unsigned char * pCharInfo;
  pFont=pFont;;
  pCharInfo = _GetpCharInfo(GUI_Context.pAFont, c);
