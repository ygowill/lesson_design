#line 1 "..\\code\\uCGUI\\Core\\GUI_UC.c"
...
#line 21 "..\\code\\uCGUI\\Core\\GUI_UC.c"
...
unsigned short GUI_UC__GetCharCodeInc(const char  ** ps) {
  ...
  s   = *ps;
  r   = GUI_Context . pUC_API->pfGetCharCode(s);
  s  += GUI_Context . pUC_API->pfGetCharSize(s);
  *ps = s;
  return r;
}
...
int GUI_UC__NumChars2NumBytes(const char  * s, int NumChars) {
  ...
  while (NumChars--) {
    CharSize = GUI_Context . pUC_API->pfGetCharSize(s);
    s += CharSize;    
    NumBytes += CharSize;
  }
  return NumBytes;
...
int GUI_UC__NumBytes2NumChars(const char  * s, int NumBytes) {
  ...
  while (NumBytes > Bytes) {
    CharSize = GUI_Context . pUC_API->pfGetCharSize(s + Bytes);
    Bytes += CharSize;
    Chars++;
  }
  return Chars;
