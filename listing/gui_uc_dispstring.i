#line 1 "..\\code\\uCGUI\\Core\\GUI_UC_DispString.c"
...
#line 21 "..\\code\\uCGUI\\Core\\GUI_UC_DispString.c"
...
#line 22 "..\\code\\uCGUI\\Core\\GUI_UC_DispString.c"
...
static int _GetLineDistX(const unsigned short  *s, int MaxNumChars) {
  int Dist =0;
  if (s) {
    unsigned short Char;
...
static int _GetLineLen(const unsigned short  *s, int MaxLen) {
  int Len =0;
  if (!s)
    return 0;
  {
...
static void _DispLine_UC(const unsigned short  *s, int Len, const GUI_RECT *pRect) {
...
static void _DispLine(const unsigned short  *s, int Len, const GUI_RECT* pr) {
  GUI_RECT r;
  r = *pr;

  r . x0 += GUI_Context . xOff; r . y0 += GUI_Context . yOff;
  r . x1 += GUI_Context . xOff; r . y1 += GUI_Context . yOff;
  { if (WM__InitIVRSearch(&r)) do { {

     GUI_Context.DispPosX = r.x0;
     GUI_Context.DispPosY = r.y0;
     _DispLine_UC(s, Len, &r);     

  } } while (WM__GetNextIVR()); };
  GUI_Context . DispPosX -= GUI_Context . xOff; GUI_Context . DispPosY -= GUI_Context . yOff;
...
void GUI_UC_DispString(const unsigned short  *s) {
  ...
  for (; *s; s++) {
    ...
    switch (GUI_Context.TextAlign & (3<<0)) { 
    case (2<<0): xAdjust= xLineSize/2; break;
    case (1<<0):  xAdjust= xLineSize; break;
    default:            xAdjust= 0;
    }
