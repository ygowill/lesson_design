#line 1 "..\\code\\uCGUI\\AntiAlias\\GUIAAChar.c"
...
#line 21 "..\\code\\uCGUI\\AntiAlias\\GUIAAChar.c"
...
#line 25 "..\\code\\uCGUI\\AntiAlias\\GUIAAChar.c"
...
#line 26 "..\\code\\uCGUI\\AntiAlias\\GUIAAChar.c"
...
typedef void tSetPixelAA(int x, int y, unsigned char Intens);
...
static const unsigned char Bit2Mask0[] = {1<<7, 1<<5, 1<<3, 1<<1};
static const unsigned char Bit2Mask1[] = {1<<6, 1<<4, 1<<2, 1<<0};
...
static void Draw(int x0, int y0, int XSize, int YSize, int BytesPerLine, const unsigned char*pData) {
    int x, y;
    tSetPixelAA* pfSetPixelAA;
    pfSetPixelAA = (GUI_Context.TextMode & (1<<1))
                 ? LCD_SetPixelAA : LCD_SetPixelAA_NoTrans;
	  for (y=0; y<YSize; y++) {
  	  const unsigned char* pData0 = pData;
	    const unsigned char* pData1 = pData+BytesPerLine;
  	  for (x=0; x<XSize; x++) {
        int PixelCnt=0;
        ...
        switch (PixelCnt) {
        case 4: GUI_Context . pLCD_HL->pfDrawPixel(x0+x,y0+y);      break;
...
void GUIPROPAA_DispChar(unsigned short c) {
  int BytesPerLine;
...
int GUIPROPAA_GetCharDistX(unsigned short c) {
  int r;
  const GUI_FONT_PROP* pProp = GUIPROP_FindChar(GUI_Context.pAFont->p.pProp, c);
...
char GUIPROPAA_IsInFont(const GUI_FONT * pFont, unsigned short c) {
