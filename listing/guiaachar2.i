#line 1 "..\\code\\uCGUI\\AntiAlias\\GUIAAChar2.c"
...
#line 21 "..\\code\\uCGUI\\AntiAlias\\GUIAAChar2.c"
...
#line 25 "..\\code\\uCGUI\\AntiAlias\\GUIAAChar2.c"
...
#line 26 "..\\code\\uCGUI\\AntiAlias\\GUIAAChar2.c"
...
static unsigned short OldColorIndex, OldBkColorIndex;
...
static void DrawNoTrans(int x0, int y0, int XSize, int YSize, int BytesPerLine, const unsigned char*pData) {
  if ((OldColorIndex   != GUI_Context . LCD . aColorIndex16[1]) || 
      (OldBkColorIndex != GUI_Context . LCD . aColorIndex16[0])) {
    int i;
    unsigned short BkColorIndex = GUI_Context . LCD . aColorIndex16[0];
    unsigned short ColorIndex   = GUI_Context . LCD . aColorIndex16[1];
...
static void DrawTrans(int x0, int y0, int XSize, int YSize, int BytesPerLine, const unsigned char*pData) {
  int x, y;
  for (y = 0; y < YSize; y++) {
    const unsigned char *pData0 = pData;
    unsigned char Rem = XSize & 3;
...
void GUIPROP_AA2_DispChar(unsigned short c) {
  int BytesPerLine;
  ...
  if (pProp) {
    ...
    if (DrawMode & (1<<1)) {
      DrawTrans(GUI_Context.DispPosX, 
                ...
                BytesPerLine,
                (unsigned char const*)pCharInfo->pData
      );
    } else {
...
int GUIPROP_AA2_GetCharDistX(unsigned short c) {
  const GUI_FONT_PROP* pProp = GUIPROP_FindChar(GUI_Context.pAFont->p.pProp, c);
...
char GUIPROP_AA2_IsInFont(const GUI_FONT * pFont, unsigned short c) {
