#line 1 "..\\code\\uCGUI\\Core\\GUIAlloc.c"
...
#line 21 "..\\code\\uCGUI\\Core\\GUIAlloc.c"
...
#line 22 "..\\code\\uCGUI\\Core\\GUIAlloc.c"
...
#line 24 "..\\code\\uCGUI\\Core\\GUIAlloc.c"
#line 25 "..\\code\\uCGUI\\Core\\GUIAlloc.c"
...
typedef union {
  int aintHeap[1024*5 / 4];    
  unsigned char  abHeap[1024*5];
} GUI_HEAP;
...
static tBlock aBlock[(2 + 1024*5 / 32)];

struct {
  int       NumUsedBlocks, NumFreeBlocks, NumFreeBlocksMin;  
  signed short NumUsedBytes,  NumFreeBytes,  NumFreeBytesMin;
} GUI_ALLOC;
...
static signed short _Size2LegalSize(signed short size) {
  return (size + ((1 << 2) - 1)) & ~((1 << 2) - 1);
}
...
static signed short _GetSize(signed short  hMem) {
...
static void _Free(signed short hMem) {
  signed short Size;
  ;
...
static signed short _FindFreeHandle(void) {
  int i;
  for (i=1; i< (2 + 1024*5 / 32); i++) {
    if (aBlock[i].Size ==0)
	  return i;
  }
  ;
...
static signed short _FindHole(signed short Size) {
  int i, iNext;
  ...
  if (1024*5 - (aBlock[i].Off+aBlock[i].Size) >= Size) {
    return i;
  }
  return -1;
}
...
static signed short _CreateHole(signed short Size) {
  int i, iNext;
  int r = -1;
  for (i=0; (iNext =aBlock[i].Next) !=0; i= iNext) {
    signed short NumFreeBytes = aBlock[iNext].Off- (aBlock[i].Off+aBlock[i].Size);
    if (NumFreeBytes < Size) {
      signed short NumBytesBeforeBlock = aBlock[iNext].Off - (aBlock[i].Off+aBlock[i].Size);
      if (NumBytesBeforeBlock) {
        unsigned char* pData = &GUI_Heap.abHeap[aBlock[iNext].Off];
...
static signed short _Alloc(signed short size) {
  signed short hMemNew, hMemIns;
  _CheckInit();
  ...
  {
    signed short Off = aBlock[hMemIns].Off + aBlock[hMemIns].Size;
...
void GUI_ALLOC_Init(void) {
  ;
  GUI_ALLOC.NumFreeBlocksMin = GUI_ALLOC.NumFreeBlocks = (2 + 1024*5 / 32)-1;
  GUI_ALLOC.NumFreeBytesMin  = GUI_ALLOC.NumFreeBytes  = 1024*5;
...
void* GUI_ALLOC_h2p(signed short  hMem) {
  ...
  return (void*)&GUI_Heap . abHeap[aBlock[hMem]. Off];
}
...
signed short GUI_ALLOC_GetMaxSize(void) {
  ...
  NumFreeBytes = (1024*5 - (aBlock[i].Off+aBlock[i].Size));
  if (NumFreeBytes > r) {
    r = NumFreeBytes;
  }
  ;
...
#line 499 "..\\code\\uCGUI\\Core\\GUIAlloc.c"
...
void GUI_ALLOC_FreePtr(signed short *ph) {
  ;
  GUI_ALLOC_Free(*ph);
  *ph =0;
  ;
