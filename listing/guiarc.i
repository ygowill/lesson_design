#line 1 "..\\code\\uCGUI\\Core\\GUIArc.c"
...
#line 33 "..\\code\\uCGUI\\Core\\GUIArc.c"
...
#line 34 "..\\code\\uCGUI\\Core\\GUIArc.c"
...
#line 35 "..\\code\\uCGUI\\Core\\GUIArc.c"
...
static void _CalcX(int*px, int y, unsigned long r2) {
  int x =*px;
  unsigned long y2 = (unsigned long)y*(unsigned long)y;
  unsigned long r2y2 = r2-y2;
  unsigned long x2;
  if (y2>=r2) {
    *px=0;
		return;
  ...
  do {
    x++;
    x2 =(unsigned long)x*(unsigned long)x;
	} while (x2 < r2y2);
	*px = x-1;
}
...
static void _DrawArc(int x0, int y0, int rx, int ry, int Angle0, int Angle1, int xMul, int yMul) {
  ...
  unsigned long   ri2 = ri*ri;
  unsigned long   ro2 = ro*ro;
	int y, yMax, yMin;
	afy[0] = ri*sin0;
	afy[1] = ro*sin0;
	afy[2] = ri*sin1;
  ...
  int xMinDisp, xMaxDisp, xMin=0,xMax=0;
    for (y=yMax; y>=yMin; y--) {
      ...
      if (xMul>0)
        GUI_Context . pLCD_HL->pfDrawHLine(xMinDisp+x0, yMul*y+y0, xMaxDisp+x0);
      else
        GUI_Context . pLCD_HL->pfDrawHLine(-xMaxDisp+x0, yMul*y+y0, -xMinDisp+x0);
    }
	}
#line 149 "..\\code\\uCGUI\\Core\\GUIArc.c"
  ry=ry;;
}
