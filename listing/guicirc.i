#line 1 "..\\code\\uCGUI\\Core\\GUICirc.c"
...
#line 35 "..\\code\\uCGUI\\Core\\GUICirc.c"
...
#line 36 "..\\code\\uCGUI\\Core\\GUICirc.c"
...
static void Draw8Point(int x0,int y0, int xoff, int yoff) {
  GUI_Context . pLCD_HL->pfDrawPixel(x0+xoff,y0+yoff);
  GUI_Context . pLCD_HL->pfDrawPixel(x0-xoff,y0+yoff);
  GUI_Context . pLCD_HL->pfDrawPixel(x0+yoff,y0+xoff);
  GUI_Context . pLCD_HL->pfDrawPixel(x0+yoff,y0-xoff);
  if (yoff) {
    GUI_Context . pLCD_HL->pfDrawPixel(x0+xoff,y0-yoff);
    GUI_Context . pLCD_HL->pfDrawPixel(x0-xoff,y0-yoff);
...
void GL_FillCircle(int x0, int y0, int r) {
  ...
  signed long sqmax = (signed long)r*(signed long)r+(signed long)r/2;
  signed long x=r;
  GUI_Context . pLCD_HL->pfDrawHLine(x0-r,y0,x0+r);
  for (i=1; i<= imax; i++) {
    if ((i*i+x*x) >sqmax) {
       
      if (x>imax) {
        GUI_Context . pLCD_HL->pfDrawHLine (x0-i+1,y0+x, x0+i-1);
        GUI_Context . pLCD_HL->pfDrawHLine (x0-i+1,y0-x, x0+i-1);
      }
      x--;
    }
     
    GUI_Context . pLCD_HL->pfDrawHLine(x0-x,y0+i, x0+x);
    GUI_Context . pLCD_HL->pfDrawHLine(x0-x,y0-i, x0+x);
  }
}
...
void GL_FillEllipse(int x0, int y0, int rx, int ry) {
  ...
  x = rx;
  for (y=0; y<=ry; y++) {
    SumY =((signed long)(rx*rx))*((signed long)(y*y));  
    while (Sum = SumY + ((signed long)(ry*ry))*((signed long)(x*x)),
           (x>0) && (Sum>OutConst))
    {
      x--;
    }
    GUI_Context . pLCD_HL->pfDrawHLine(x0-x, y0+y, x0+x);
    if (y)
      GUI_Context . pLCD_HL->pfDrawHLine(x0-x, y0-y, x0+x);
  }
}
