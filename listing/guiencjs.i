#line 1 "..\\code\\uCGUI\\Core\\GUIEncJS.c"
...
#line 23 "..\\code\\uCGUI\\Core\\GUIEncJS.c"
...
#line 24 "..\\code\\uCGUI\\Core\\GUIEncJS.c"
...
static unsigned short DB2SJIS(unsigned char Byte0, unsigned char Byte1) {
...
static int _GetLineDistX_SJIS(const char  *s, int Len) {
  ...
  if (s) {
    unsigned char c0;
    while (((c0=*(const unsigned char*)s) !=0) && Len >=0) {
      s++; Len--;
      if (c0 > 127) {
        unsigned char  c1 = *(const unsigned char*)s++;
        Len--;
...
static int _GetLineLen_SJIS(const char  *s, int MaxLen) {
  int Len =0;
  unsigned char c0;
  while (((c0=*(const unsigned char*)s) !=0) && Len < MaxLen) {
    s++;
    if (c0 > 127) {
      Len++; s++;
    } else {
