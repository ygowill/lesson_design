#line 1 "..\\code\\uCGUI\\Core\\LCDAA.c"
...
#line 23 "..\\code\\uCGUI\\Core\\LCDAA.c"
...
#line 24 "..\\code\\uCGUI\\Core\\LCDAA.c"
...
#line 25 "..\\code\\uCGUI\\Core\\LCDAA.c"
#line 26 "..\\code\\uCGUI\\Core\\LCDAA.c"
...
LCD_COLOR LCD_AA_MixColors(LCD_COLOR Color, LCD_COLOR BkColor, unsigned char Intens) {
  ...
  signed long R = (Color&0xff)*Intens;
  signed long G = (Color&0xff00)*Intens;
  signed long B = (Color&0xff0000)*Intens;
...
void LCD_SetPixelAA(int x, int y, unsigned char Intens) {
  ...
  if (Intens >= 15) {
    ...
    LCD_COLOR Color = LCD_Index2Color(GUI_Context . LCD . aColorIndex16[1]);
    LCD_COLOR BkColor =  LCD_GetPixelColor(x,y);
    Color = LCD_AA_MixColors(Color, BkColor, Intens);
    LCD_L0_SetPixelIndex(x,y, LCD_Color2Index(Color));
  }
}
...
void LCD_SetPixelAA_NoTrans(int x, int y, unsigned char Intens) {
  ...
  if (x < GUI_Context . ClipRect . x0) return; if (x > GUI_Context . ClipRect . x1) return;;
  if (Intens == 0) {
    LCD_L0_SetPixelIndex(x,y, GUI_Context . LCD . aColorIndex16[0]);
  } else if (Intens == 15) {
    LCD_L0_SetPixelIndex(x,y, GUI_Context . LCD . aColorIndex16[1]);
  } else {
    LCD_COLOR Color = LCD_AA_MixColors(LCD_Index2Color(GUI_Context . LCD . aColorIndex16[1]),
                                   LCD_Index2Color(GUI_Context . LCD . aColorIndex16[0]),
                                   Intens);
    LCD_L0_SetPixelIndex(x,y,LCD_Color2Index(Color));
  }
}
