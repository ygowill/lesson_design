#line 1 "..\\code\\uCGUI\\Core\\LCDRLE8.c"
...
#line 22 "..\\code\\uCGUI\\Core\\LCDRLE8.c"
...
#line 23 "..\\code\\uCGUI\\Core\\LCDRLE8.c"
#line 24 "..\\code\\uCGUI\\Core\\LCDRLE8.c"
...
static struct {
  int x,y;
  const unsigned char  * pPixel;
  const unsigned char  * pPixelStart;
} Cache;
...
static void _DrawBitmap_RLE8(int x0,int y0,int xsize, int ysize, const unsigned char  * pPixel, const LCD_LOGPALETTE  * pLogPal, int xMag, int yMag) {
  unsigned short aColorIndex[2];
  unsigned short PixelIndex;
  int xi,y;
  int xL, yL;
  const unsigned char  * pPixelOrg = pPixel;
  char NoTrans = !(GUI_Context.DrawMode & (1<<1));
  const unsigned short* pTrans =0;
  char IsMagnified = ((yMag | xMag) != 1);
  aColorIndex[0] = GUI_Context . LCD . aColorIndex16[0];
  aColorIndex[1] = GUI_Context . LCD . aColorIndex16[1];
   
  if ((pLogPal) && (pLogPal->pPalEntries)) {
    if ((pTrans = LCD_GetpPalConvTable(pLogPal)) == 0) {
      return;
    }
  }
  ...
  for (; y < ysize; ) {
    unsigned char Cmd  = *pPixel++;
    unsigned char Data = *pPixel++;
    if (Cmd) {
       
      Cache.pPixel = pPixel-2;
      Cache.x = xi;
      Cache.y = y;
      GUI_Context . LCD . aColorIndex16[1] = pTrans ? *(pTrans+Data) : Data;
      while (Cmd) {
        int xi1 = xi+Cmd;
        if (xi1>=xsize)
          xi1 = xsize;
              ...
              if (y + y0>= GUI_Context.ClipRect.y0)
                if (x >= GUI_Context.ClipRect.x0)
                  if (x <= GUI_Context.ClipRect.x1)
                    LCD_L0_SetPixelIndex(x, y + y0, PixelIndex);
  ...
  GUI_Context . LCD . aColorIndex16[0] = aColorIndex[0];
  GUI_Context . LCD . aColorIndex16[1] = aColorIndex[1];
}
