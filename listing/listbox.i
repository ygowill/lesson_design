#line 1 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
...
#line 21 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
...
#line 22 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
...
#line 23 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
#line 1 "..\\code\\uCGUI\\Widget\\LISTBOX_Private.h"
...
#line 24 "..\\code\\uCGUI\\Widget\\LISTBOX_Private.h"
#line 25 "..\\code\\uCGUI\\Widget\\LISTBOX_Private.h"
#line 26 "..\\code\\uCGUI\\Widget\\LISTBOX_Private.h"
#line 27 "..\\code\\uCGUI\\Widget\\LISTBOX_Private.h"
...
#line 49 "..\\code\\uCGUI\\Widget\\LISTBOX_Private.h"
...
typedef struct {
  ...
  signed short Sel;                         
  unsigned char Flags;
  unsigned char  ScrollbarWidth;
  unsigned short ItemSpacing;
} LISTBOX_Obj;
...
#line 24 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
#line 25 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
#line 26 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
#line 1 "..\\code\\GUI\\GUI_inc\\GUI_Protected.h"
...
#line 27 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
#line 28 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
...
static int _GetItemSizeX(LISTBOX_Handle hObj, const LISTBOX_Obj* pObj, unsigned Index) {
  ...
  if (xSize == 0) {
    ...
    xSize    = _CallOwnerDraw(hObj, pObj, 1, Index);
...
static int _GetItemSizeY(LISTBOX_Handle hObj, const LISTBOX_Obj* pObj, unsigned Index) {
  ...
  if (ySize == 0) {
    const GUI_FONT * pOldFont;
    pOldFont = GUI_SetFont(pObj->Props.pFont);
    ySize    = _CallOwnerDraw(hObj, pObj, 2, Index);
    GUI_SetFont(pOldFont);
  }
  if (pItem) {
    pItem->ySize = ySize;
...
static int _GetContentsSizeX(LISTBOX_Handle hObj) {
  ...
  int Result = 0;
  pObj = (LISTBOX_Obj*) GUI_ALLOC_h2p(hObj);
...
static void _NotifyOwner(GUI_HWIN hObj, int Notification) {
  WM_MESSAGE Msg = {0};
  GUI_HWIN hOwner;
  LISTBOX_Obj* pObj    = (LISTBOX_Obj*) GUI_ALLOC_h2p(hObj);
...
int LISTBOX_OwnerDraw(const WIDGET_ITEM_DRAW_INFO* pDrawItemInfo) {
  switch (pDrawItemInfo->Cmd) {
    case 1: {
      ...
      const char* s;
      int DistX;
      pObj = (LISTBOX_Obj*) GUI_ALLOC_h2p(pDrawItemInfo->hWin);
      pOldFont = GUI_SetFont(pObj->Props.pFont);
    ...
    case 0: {
      ...
      IsDisabled = (pItem->Status & (1 << 1)) ? 1 : 0;
      IsSelected = (pItem->Status & (1 << 0)) ? 1 : 0;
      if (pObj->Flags & (1<<2)) {
        if (IsDisabled) {
          ColorIndex = 3;
        } else {
        ...
        if (IsDisabled) {
          ...
          if (ItemIndex == pObj->Sel) {
            ColorIndex = (pObj->Widget.State & (1<<0)) ? 2 : 1;
          } else {
            ColorIndex = 0;
          }
        }
      ...
      if ((pObj->Flags & (1<<2)) && (ItemIndex == pObj->Sel)) {
        GUI_RECT rFocus;
        rFocus.x0 = pDrawItemInfo->x0;
        rFocus.y0 = pDrawItemInfo->y0;
        rFocus.x1 = r.x1;
        rFocus.y1 = pDrawItemInfo->y0 + FontDistY - 1;
        LCD_SetColor(0xFFFFFF - pObj->Props.aBackColor[ColorIndex]);
...
void LISTBOX__SetScrollbarWidth(LISTBOX_Handle hObj, const LISTBOX_Obj* pObj) {
  GUI_HWIN hBarH, hBarV;
  int Width;
  Width = pObj->ScrollbarWidth;
  if (Width == 0) {
    Width = SCROLLBAR_GetDefaultWidth();
  }
  hBarH = WM_GetDialogItem(hObj, 0xFF);
  hBarV = WM_GetDialogItem(hObj, 0xFE);
...
static void _ToggleMultiSel(LISTBOX_Handle hObj, LISTBOX_Obj* pObj, int Sel) {
  if (pObj->Flags & (1<<2)) {
    signed short hItem = GUI_ARRAY_GethItem(&pObj->ItemArray, Sel);
    if (hItem) {
...
#line 746 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
...
static void _LISTBOX_Callback(WM_MESSAGE*pMsg) {
  ...
  case 12:
    _OnTouch(hObj, pMsg);
    return;
#line 813 "..\\code\\uCGUI\\Widget\\LISTBOX.c"
  case 11:
    _FreeAttached(pObj);
    break;        
  case 14:
...
void LISTBOX_SetSel (LISTBOX_Handle hObj, int NewSel) {
  if (hObj) {
    ...
    if (NewSel < 0) {
      ...
      signed short hItem = GUI_ARRAY_GethItem(&pObj->ItemArray, NewSel);
      if (hItem) {
        LISTBOX_ITEM* pItem = (LISTBOX_ITEM*)GUI_ALLOC_h2p(hItem);
        if (pItem->Status & (1 << 1)) {
          NewSel = -1;
