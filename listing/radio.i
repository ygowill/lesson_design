#line 1 "..\\code\\uCGUI\\Widget\\RADIO.c"
...
#line 21 "..\\code\\uCGUI\\Widget\\RADIO.c"
...
#line 22 "..\\code\\uCGUI\\Widget\\RADIO.c"
...
#line 23 "..\\code\\uCGUI\\Widget\\RADIO.c"
...
GUI_COLOR         RADIO__DefaultTextColor       = 0x000000;
const GUI_FONT * RADIO__pDefaultFont = &GUI_Font13_1;
const GUI_BITMAP* RADIO__apDefaultImage[]       = {&RADIO__abmRadio[0], &RADIO__abmRadio[1]};
const GUI_BITMAP* RADIO__pDefaultImageCheck     = &RADIO__bmCheck;
...
#line 97 "..\\code\\uCGUI\\Widget\\RADIO.c"
...
static void _OnPaint(RADIO_Handle hObj, RADIO_Obj* pObj) {
  ...
  unsigned char SpaceAbove, CHeight, FocusBorder;

   
  WIDGET__GetClientRect(&pObj->Widget, &rFocus);
  HasFocus  = (pObj->Widget.State & (1<<0)) ? 1 : 0;
  pBmRadio  = pObj->apBmRadio[WM__IsEnabled(hObj)];
  pBmCheck  = pObj->pBmCheck;
  rFocus.x1 = pBmRadio->XSize + 2 * 2 - 1;
  ...
  {
    if (pObj->BkColor != 0xFFFFFFF) {
      LCD_SetBkColor(pObj->BkColor);
    } else {
      LCD_SetBkColor(0xC0C0C0);
    }
    GUI_Clear();
  }
  ...
  for (i = 0; i < pObj->NumItems; i++) {
    ...
    GUI_DrawBitmap(pBmRadio, 2, 2 + y);
     
    if (pObj->Sel == i) {
      GUI_DrawBitmap(pBmCheck, 2 +  (pBmRadio->XSize - pBmCheck->XSize) / 2, 
                               2 + ((pBmRadio->YSize - pBmCheck->YSize) / 2) + y);
    }
                            ...
                            int WinFlags, int ExFlags, int Id, int NumItems, int Spacing)
{
  ...
  Height   = RADIO__apDefaultImage[0]->YSize + 2 * 2;
  ...
  if (xSize == 0) {
    xSize  = RADIO__apDefaultImage[0]->XSize + 2 * 2;
  }
  ...
  if (hObj) {
    ...
    for (i = 0; i < NumItems; i++) {
      GUI_ARRAY_AddItem(&pObj->TextArray, 0, 0);
    }
     
    ExFlags &= (1<<8);
    WIDGET__Init(&pObj->Widget, Id, (1<<4) | ExFlags);
...
#line 469 "..\\code\\uCGUI\\Widget\\RADIO.c"
