#line 1 "..\\code\\uCGUI\\Widget\\SCROLLBAR_Create.c"
...
#line 21 "..\\code\\uCGUI\\Widget\\SCROLLBAR_Create.c"
...
SCROLLBAR_Handle SCROLLBAR_CreateAttached(GUI_HWIN hParent, int SpecialFlags) {
  ...
  if (SpecialFlags & (1<<3)) {
    Id = 0xFE;
    WinFlags = (1<<1) | (1<<3) | (1<<7) | (1<<10) | (1<<8);
  } else {
    Id = 0xFF;
    WinFlags = (1<<1) | (1<<3) | (1<<8) | (1<<9) | (1<<7);
  }
  hThis = SCROLLBAR_CreateEx(0, 0, 0, 0, hParent, WinFlags, SpecialFlags, Id);
  WM_NotifyParent(hThis, 6);
  return hThis;
}
